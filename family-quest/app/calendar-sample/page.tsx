...
type Event={time:string;title:string;who:string;icon:string}; type Todo={text:string;who:string;pts:number;done:boolean};
...
export default function Page(){
 const [tab,setTab]=useState('home'),[filter,setFilter]=useState('全員'),[events,setEvents]=useState(baseEvents),[todos,setTodos]=useState(baseTodos),[open,setOpen]=useState(false),[type,setType]=useState('予定'),[title,setTitle]=useState(''),[who,setWho]=useState('全員'),[time,setTime]=useState('18:00');
 useEffect(()=>{try{const e=localStorage.getItem('fq-events'),t=localStorage.getItem('fq-todos');if(e)setEvents(JSON.parse(e));if(t)setTodos(JSON.parse(t))}catch{}},[]);
 useEffect(()=>{localStorage.setItem('fq-events',JSON.stringify(events));localStorage.setItem('fq-todos',JSON.stringify(todos))},[events,todos]);
 const shown=filter==='全員'?events:events.filter(e=>e.who.includes(filter)||e.who==='全員');
 ...
 function add(){if(!title.trim())return;if(type==='予定')setEvents(v=>[...v,{time,title,who,icon:'📌'}].sort((a,b)=>a.time.localeCompare(b.time)));else setTodos(v=>[...v,{text:title,who,pts:10,done:false}]);setTitle('');setOpen(false)}
 return <main style={main}><div style={wrap}>
 <header style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><div><small>FAMILY OS · β</small><h1 style={{margin:'3px 0'}}>わが家クエスト</h1><p style={{margin:'0 0 12px',color:'#64748b'}}>予定・宿題・成長をひとつに</p></div><button style={plus} onClick={()=>setOpen(true)}>＋</button></header>
 <div style={{display:'flex',gap:8,overflowX:'auto',padding:'8px 0 14px'}}>{people.map(p=><button key={p[0]} onClick={()=>setFilter(p[0])} style={{...chip,background:filter===p[0]?'#183153':'white',color:filter===p[0]?'white':'#183153'}}>{p[1]} {p[0]}</button>)}</div>
 {tab==='home'&&<><section style={card}><h2>☀️ 今日の作戦</h2>{shown.map((e,i)=><div style={row} key={i}><b>{e.time}</b><span>{e.icon}</span><div><b>{e.title}</b><small style={{display:'block'}}>{e.who}</small></div></div>)}</section>
 ...
 {tab==='calendar'&&<section style={card}><h2>📅 2026年9月</h2><div style={grid}>{['月','火','水','木','金','土','日'].map(x=><b key={x}>{x}</b>)}{Array.from({length:35},(_,i)=>i===0?'':i).map((d,i)=><div key={i} style={{...day,border:d===24?'2px solid #2878e8':'1px solid #e5e7eb'}}>{d}{d===24&&<><i style={tag}>仕事</i><i style={{...tag,background:'#efe2ff'}}>剣道</i></>}</div>)}</div></section>}
 ...
 {open&&<div style={shade}><div style={{...card,width:'min(88vw,410px)',margin:0}}><h2>＋ 新しく追加</h2><div style={{display:'flex',gap:8}}>{['予定','ミッション'].map(x=><button style={{...chip,background:type===x?'#2878e8':'#eef2f7',color:type===x?'white':'#183153'}} key={x} onClick={()=>setType(x)}>{x}</button>)}</div><input style={input} value={title} onChange={e=>setTitle(e.target.value)} placeholder={type==='予定'?'例：歯医者・学校行事':'例：算数プリント'}/><select style={input} value={who} onChange={e=>setWho(e.target.value)}>{people.map(p=><option key={p[0]}>{p[0]}</option>)}</select>{type==='予定'&&<input style={input} type="time" value={time} onChange={e=>setTime(e.target.value)}/>}<button style={{...plus,width:'100%',borderRadius:12,marginTop:12}} onClick={add}>追加する</button><button style={{...chip,width:'100%',marginTop:8}} onClick={()=>setOpen(false)}>閉じる</button><small style={{display:'block',marginTop:10,color:'#64748b'}}>このβ版は、この端末のブラウザに保存します。Googleカレンダー同期はまだ行いません。</small></div></div>}
 <nav style={nav}>{[['home','🏠','ホーム'],['calendar','📅','予定'],['todo','🎯','ミッション'],['record','📊','成長']].map(x=><button key={x[0]} onClick={()=>setTab(x[0])} style={{border:0,background:'white',padding:9,color:tab===x[0]?'#2878e8':'#64748b',fontWeight:700}}><div style={{fontSize:20}}>{x[1]}</div><small>{x[2]}</small></button>)}</nav></main>}
...
const main:React.CSSProperties={minHeight:'100vh',background:'#f3f6fb',color:'#183153',fontFamily:'system-ui,sans-serif',paddingBottom:86};
...
const nav:React.CSSProperties={position:'fixed',bottom:0,left:0,right:0,maxWidth:520,margin:'auto',display:'grid',gridTemplateColumns:'repeat(4,1fr)',background:'white',borderTop:'1px solid #dfe7f1',zIndex:20};
const grid:React.CSSProperties={display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:4,textAlign:'center',fontSize:11};
const day:React.CSSProperties={height:57,borderRadius:8,padding:4,textAlign:'left'};
