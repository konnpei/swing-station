...
本日の市場データ:
- 日付: {TODAY} ({WEEKDAY_JP})
- 日経225（前営業日終値）: {data["latest"]["close"]:,}円（前々営業日比 {sign}{abs(int(data["diff"])):,}円 / {data["pct"]:+.2f}%）
...
{{
  ...
  "x_teaser_3line": "note記事の宣伝用にXへ投稿する一言（30文字程度、厳守）。note記事の見出しをそのまま転載しない。一人称で今日の記事の中身を一言でのぞかせる程度。絵文字は可、ハッシュタグやリンクは含めない（別途本文に付け足すため）",
  {note_body_field}
...
if __name__ == "__main__":
    ...
    print("Fetching Bloomberg/Nikkei headlines...")
    try:
        top_headlines = fetch_bloomberg_nikkei_headlines(limit=3)
        print(f"Headlines fetched: {len(top_headlines)}件")
    except Exception as e:
